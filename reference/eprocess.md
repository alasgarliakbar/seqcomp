...
- `e_pq`, `e_qp`:
  ...
  One-sided e-processes. `e_pq` tests H_0^w(p, q): whether forecaster
  `p` outperforms `q`; `e_qp` tests H_0^w(q, p): whether forecaster `q`
  outperforms `p`.

- `log_e_pq`, `log_e_qp`:

  Log-scale values of the e-processes, clipped at log(clip_max).
...
\\m(s, v)\\ is the Gamma-Exponential mixture function (Proposition EC.3,
CR24).
...
CR24. For Brier score differences in `[-1,1]`: c = 2. For Winkler scores
